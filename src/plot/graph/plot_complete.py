...
# y 軸：実行時間（ミリ秒）
# 実験データは完全グラフの結果を使用
def plot_complete(cpu_size_limit = None):
    ...
    # 実験データの読み込み
    files = glob.glob("./../../graph/result/complete_graph/*.txt")
    ...
    plt.title(f"1000 頂点 499500 辺の完全グラフに対する実験結果")
    plt.xlabel("並列度")
    plt.ylabel("実行時間 [ms]")
    plt.tight_layout() # レイアウトの調整

    # グラフの保存
    save_path = "plot_complete.png"
    if cpu_size_limit is not None:
        save_path = f"plot_complete_limit_{cpu_size_limit}.png"
...
def main():
    path1 = plot_complete() # 全ての並列度に対するグラフを作成
    path2 = plot_complete(cpu_size_limit = 100) # 特定の並列度以下のグラフを作成
